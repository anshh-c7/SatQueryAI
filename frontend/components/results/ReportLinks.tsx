...
export const ReportLinks: React.FC<ReportLinksProps> = ({ report, conversation, imagePreviews = [], isReportPage = false }) => {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-xl border border-stone-200 bg-stone-100/80 p-3 dark:border-white/10 dark:bg-[#1C1917]">
      ...
      <div className="flex flex-wrap items-center gap-2">
        {isReportPage ? (
          <button type="button" onClick={() => window.history.back()} className="apple-interactive px-3 py-1.5 rounded-lg bg-stone-200/80 dark:bg-stone-800 text-primary hover:bg-stone-300 dark:hover:bg-stone-700 font-medium flex items-center gap-1.5 transition-colors">
            <LogOut className="w-3.5 h-3.5" />
            <span>Escape</span>
          </button>
        ) : (
          <a href={`/analysis/${report.report_id}`} target="_blank" rel="noopener noreferrer" onClick={rememberReportImages} className="apple-interactive px-3 py-1.5 rounded-lg bg-stone-200/80 dark:bg-stone-800 text-primary hover:bg-stone-300 dark:hover:bg-stone-700 font-medium flex items-center gap-1.5 transition-colors">
