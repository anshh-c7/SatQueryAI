    ...
    <section className="space-y-4 rounded-2xl border border-stone-300/70 bg-white/65 p-4 shadow-subtle dark:border-white/10 dark:bg-[#171512]/80 sm:p-5">
      ...
      <div className="grid min-w-0 gap-3 md:grid-cols-2">
        <div className="min-w-0 rounded-xl border border-stone-200/80 bg-white/60 p-3 dark:border-white/10 dark:bg-[#1C1917]">
          <div className="mb-3 flex items-center justify-between gap-2"><span className="text-xs font-semibold text-primary">Changed area</span><span className="font-mono text-xs text-accent">{changedPercent.toFixed(2)}%</span></div>
          <div className="flex h-40 items-end gap-2 rounded-lg bg-stone-100/70 px-3 pb-3 pt-5 dark:bg-black/20">
            {[Math.max(4, changedPercent * 0.35), Math.max(4, changedPercent * 0.65), Math.max(4, changedPercent)].map((height, index) => <div key={index} className="flex-1 rounded-t-md bg-accent/80" style={{ height: `${Math.min(100, height)}%` }} />)}
          </div>
          <div className="mt-2 flex justify-between text-[10px] font-mono text-secondary"><span>Area footprint</span><span>{regionCount} region{regionCount === 1 ? "" : "s"}</span></div>
        </div>
        <div className="min-w-0 rounded-xl border border-stone-200/80 bg-white/60 p-3 dark:border-white/10 dark:bg-[#1C1917]">
