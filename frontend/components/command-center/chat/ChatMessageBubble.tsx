...
import { ResultSummaryCard } from "@/components/command-center/chat/ResultSummaryCard";
...
export const ChatMessageBubble: React.FC<ChatMessageBubbleProps> = ({ message }) => {
  ...
  if (message.status === "pending") {
    return (
      <ThinkingIndicator
        ...
        activeStepIndex={message.activeStepIndex}
        backendStatus={message.backendStatus}
      />
    );
  }
    ...
    return (
      ...
      >
        <div className="max-w-[85%] rounded-2xl rounded-tr-sm bg-gradient-to-br from-[#7F4B30] to-[#B27D57] text-white px-4 py-3 text-xs shadow-[0_8px_20px_rgba(127,75,48,0.25),inset_0_1px_1px_rgba(255,255,255,0.3)] border border-[#C86D3B]/40 leading-relaxed">
          ...
          <div className="mt-1.5 text-[10px] text-white/70 text-right font-mono">
  ...
  if (message.status === "error") {
    return (
      ...
      >
        <div className="max-w-[90%] rounded-2xl rounded-tl-sm bg-rose-50/90 dark:bg-rose-950/40 backdrop-blur-md border border-rose-200 dark:border-rose-900/50 p-4 text-xs text-rose-800 dark:text-rose-200 shadow-xs space-y-2.5">
          <div className="flex items-start gap-2 text-rose-900 dark:text-rose-100 font-medium">
            <AlertTriangle className="w-4 h-4 text-rose-600 shrink-0 mt-0.5" />
            <span>{message.errorMessage || "Inference pipeline query failed."}</span>
          </div>
          <p className="text-[11px] text-secondary dark:text-[#B8AEA3]">
  ...
  return (
    ...
    >
      <div className="glass-card max-w-[90%] rounded-2xl rounded-tl-sm p-4 text-xs text-primary space-y-3 leading-relaxed">
        ...
        {/* Compact Result Summary Card for analytical responses */}
        {(hasEvidence || hasAudit) && (
          <ResultSummaryCard message={message} />
        )}
        ...
        {hasAudit && (
          <div className="flex items-center justify-between pt-2 border-t border-stone-200/60 dark:border-white/10 text-[11px]">
            <div className="flex items-center gap-1.5 text-emerald-800 dark:text-emerald-300 font-mono text-[10px]">
        ...
        <div className="text-[10px] text-secondary/60 dark:text-[#91877D] font-mono">
          {new Date(message.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
        </div>
      </div>
    </motion.div>
