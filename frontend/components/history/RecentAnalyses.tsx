...
export function RecentAnalyses({ refreshKey = 0, embedded = false }: { refreshKey?: number; embedded?: boolean }) {
  ...
  const handleDelete = async (conversation: ChatConversation) => {
    ...
    if (selected?.turnId === conversation.turnId) setSelected(null);
  ...
  return (
    <div className={embedded ? "relative min-h-0" : "relative flex items-center gap-2"}>
      ...
      {(embedded || open) && <div className={embedded ? "w-full" : "absolute right-0 top-10 z-30 w-80 rounded-xl border border-stone-300/70 bg-[#FAF6F0] p-3 shadow-xl dark:border-white/10 dark:bg-[#171512]"}>
        ...
        {items.length === 0 ? <p className="px-1 text-xs text-secondary">No saved conversations yet.</p> : <div className="max-h-[min(48vh,24rem)] space-y-1 overflow-y-auto">{items.map((item) => <div key={item.turnId} className="group flex items-start gap-2 rounded-lg px-1 py-1.5 hover:bg-black/5 dark:hover:bg-white/5"><button type="button" onClick={() => setSelected(item)} className="min-w-0 flex-1 text-left"><div className="flex items-start gap-2"><MessageCircle className="mt-0.5 h-3.5 w-3.5 shrink-0 text-accent" /><p className="truncate text-xs text-primary">{item.prompt?.content ?? "Conversation"}</p></div><p className="mt-1 pl-5 text-[10px] text-secondary">{new Date(item.createdAt).toLocaleString()}</p></button><button type="button" onClick={() => void handleDelete(item)} disabled={deletingTurnId === item.turnId} className="mt-0.5 rounded p-1 text-secondary opacity-0 transition-opacity hover:bg-rose-500/10 hover:text-rose-600 group-hover:opacity-100 focus-visible:opacity-100 disabled:opacity-50" title="Delete conversation" aria-label="Delete conversation"><Trash2 className="h-3.5 w-3.5" /></button></div>)}</div>}
      </div>}
      {selected && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4 backdrop-blur-sm" onClick={() => setSelected(null)}><div className="max-h-[90vh] w-full max-w-5xl overflow-y-auto rounded-2xl border border-stone-300/70 bg-[#FAF6F0] p-5 shadow-2xl dark:border-white/10 dark:bg-[#171512]" onClick={(event) => event.stopPropagation()}><div className="mb-4 flex items-center justify-between"><div><p className="font-serif text-xl text-primary">Saved analysis</p><p className="text-[10px] font-mono text-secondary">{new Date(selected.createdAt).toLocaleString()}</p></div><button type="button" onClick={() => setSelected(null)} className="rounded-full p-1.5 text-secondary hover:bg-black/5 hover:text-primary" aria-label="Close conversation"><X className="h-4 w-4" /></button></div><div className="mb-5 rounded-xl bg-stone-200/60 p-3 dark:bg-[#1F1B17]"><p className="mb-1 text-[10px] font-mono uppercase text-secondary">You</p><p className="text-sm text-primary">{selected.prompt?.content ?? "Prompt unavailable"}</p></div>{selected.response?.response ? <ResultsPanel data={selected.response.response} imagePreviews={selected.response.response.image_previews} /> : <p className="text-sm text-secondary">No complete model output was saved for this conversation.</p>}</div></div>}
    </div>
  );
}
