...
export const AuditModelList: React.FC<AuditModelListProps> = ({ models }) => {
  return (
    <div className="space-y-2.5">
      ...
      <div className="glass-card rounded-2xl divide-y divide-stone-200/50 overflow-hidden">
        {models.map((model, idx) => (
          <div
            key={idx}
            className="flex items-center justify-between p-3.5 text-xs hover:bg-white/40 transition-colors"
          >
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                <span className="font-semibold text-primary">{model.name}</span>
                <span className="rounded-full bg-sand-100 px-2 py-0.5 text-[10px] font-mono text-secondary border border-stone-200">
                  {model.version}
                </span>
              </div>
            </div>

            <div className="flex items-center gap-1.5 font-mono text-[11px] text-secondary bg-white/60 px-2.5 py-1 rounded-full border border-stone-200">
